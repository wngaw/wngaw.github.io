---
layout: page
title: Resume
permalink: /resume/
---

## EXPERIENCE
---
**Stealth Startup**  
_**Technical & Team Lead, Big Data**_  
Dec 2023 - Present  
- Real-time pricing engine (10,000+ tokens)
- Real-time sub-second blockchain transaction processing (100+ blockchains)
- Real-time outlier detection
- Real-time on-chain wallet tagging
- Trading signals and alpha generation
- Portfolio P&L computation
- On-chain prediction market
- Recommender systems
- Ranking systems

Technologies: Kubernetes, Redis, AWS Elastic Compute Cloud (EC2), AWS Elastic MapReduce (EMR), AWS Simple Storage Service (S3), AWS DynamoDB, Spark, Docker, Elasticsearch, Python, Java, Airflow, Jira, among others.

<br />

**BINANCE**  
_**Technical & Team Lead, Sr Data Scientist, Data Science & Algorithm**_  
Sep 2020 - Nov 2023  
[https://www.binance.com](https://www.binance.com/)  
- Marketing Data Science: customer lifetime value prediction, churn user segmentation, causal inference analysis, marketing mix modelling and competitor analysis.
- Operations Data Science: graph-based anomaly detection, user & crypto token embeddings, text intent classification, online-search re-ranking, credit-scoring, interest rate optimisation and wallet fund optimisation.
- Blockchain Data Science: compliance KYT, security monitoring, risk management, business strategy and research.
- Infrastructure Data Science: Real-time infrastructure smart alert system.
- ML Ops: data science models CICD
- Project Management: internally certified Scrum Master

Technologies: AWS Elastic Compute Cloud (EC2), AWS Elastic MapReduce (EMR), AWS Simple Storage Service (S3), AWS SageMaker, Spark, Docker, Elasticsearch, Python, Java, Airflow, Jira, among others.

<br />

**NTUC ENTERPRISE**  
_**Data Scientist, Data Science**_  
Jul 2018 - Sep 2020  
[https://www.ntuc.org.sg](https://www.ntuc.org.sg)  
- Data science team within NTUC Chief Analytics Office. Built machine learning models such as recommender systems, online search ranking, user & item graph embeddings, customer & store segmentation, assortment & promotion optimisation for Singapore's largest grocery chain.

Technologies: AWS Elastic Compute Cloud (EC2), AWS Elastic MapReduce (EMR), AWS Redshift, AWS Simple Storage Service (S3), AWS DynamoDB, Google Compute Engine, Google Cloud SQL, Google BigQuery, Google Cloud Storage, Google Kubernetes Engine, Oracle Database, LevelDB, Spark, Docker, Elasticsearch, Python, Java, Airflow, MLflow, Django, Springboot, Tableau, BitBucket, Jira, among others.

<br />

**UBER**  
_**Data Analyst, Customer Lifecycle Analytics**_  
Aug 2017 - Jul 2018  
[https://www.uber.com](https://www.uber.com)  
- Regional data engineering and data science for Uber Rides and Uber Eats, across performance marketing, CRM marketing, and analytics & insights.

Technologies: Python, SQL, Hive, Presto, Vertica, BigQuery, Tableau, Docker, Airflow, Git, Phabricator, Jenkins, and Jira, among others.

<br />

**GOOGLE**  
_**Graduate Trainee, Data & Analytics**_  
Jun 2017 - Aug 2017  
[https://www.google.com](https://www.google.com)  
- Trained by Google in data and analytics.

Technologies: Python, R, SQL, BigQuery, Tableau, Data Studio, Google Analytics, Google Adwords, and Google Adsense, among others.

<br />

**FULLERTON FUND MANAGEMENT**  
_**Analyst, Multi-Asset Strategies**_  
May 2016 - Mar 2017  
[http://www.fullertonfund.com](http://www.fullertonfund.com)  
- Quantitative investment strategies across equities, bonds, commodities, and foreign currencies. $40 billion in asset under management as at 2017.

Technologies: R, VBA, Microsoft Excel, Bloomberg Terminal, and FactSet.

<br />

**DYMON ASIA CAPITAL**  
_**Analyst, Macroeconomics Research and Strategy**_  
Jan 2016 - Mar 2016  
[https://www.dymonasia.com/](https://www.dymonasia.com/)  
- Macroeconomics investment strategies for global currencies and equity indices. $5 billion in asset under management as at 2016.

Technologies: VBA, Microsoft Excel, and Bloomberg Terminal.

<br />

**CITIBANK**  
_**Analyst, Global Markets**_  
Jan 2015 - Dec 2015  
[https://www.citigroup.com](https://www.citigroup.com)  
- Prime brokerage services for multi-billion dollar hedge funds across Asia.

Technologies: VBA, Microsoft Excel, and Bloomberg Terminal.

<br />

**Singapore Armed Forces**  
_**Infantry Commander**_  
May 2011 - Mar 2013  
[https://www.mindef.gov.sg/](https://www.mindef.gov.sg/)  
- Mandatory full-time national service.

<br />

## EDUCATION
---
**NATIONAL UNIVERSITY OF SINGAPORE**  
Master of Science: Statistics

**NANYANG TECHNOLOGICAL UNIVERSITY**  
Bachelor of Business Administration (Honours): Finance

**ESSEC BUSINESS SCHOOL**  
Overseas Exchange Program

**CFA INSTITUTE**  
Chartered Financial Analyst Level 1

## HONOURS & AWARDS
---
- Binance - Top Performer (2021, 2022)
- NTUC Enterprise - Top Performer (2019, 2020)
- Google - Won 1st Place @ Google X Carousell Challenge (2017)
- Nanyang Technological University - Ng Bok Eng Scholarship (2014)
- Serangoon Junior College - Top Overall Student Award (2010)
- Serangoon Junior College - Top Science Student Award (2010)
- Serangoon Junior College - Academic Distinction Award (2010)
- Serangoon Junior College - Top Chemistry Student Award (2010)
- Serangoon Junior College - Top Chemistry Student Award (2009)
